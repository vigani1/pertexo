...
| Failure-notification context V1 reader | Strict node primary-failure shape and additive run-level timeout primary-failure shape within immutable context `schemaVersion: 1` | Initial notification release / open-ended | R0 predecessor workers; R1 dual-reader workers with `FAILURE_NOTIFICATION_RUN_TIMEOUT_CONTEXT_ENABLED=false` (the default) on every coordinator; R2 dual-reader workers with the gate explicitly `true` on coordinator producers | `apps/worker/test/worker-config.test.ts` proves fail-safe activation parsing; `packages/workflow-model/test/failure-notification.test.ts` pins predecessor/candidate JSON fixtures; `packages/database/test/coordinator-run-store-scheduling.integration.test.ts` proves R1 suppression preserves terminal truth and R2 queued-timeout mixed-reader takeover, restart, recovery, and redelivery | Per supported database, zero intents containing `context.primaryFailure.source = 'run'`, zero related unpublished outbox rows or queue deliveries, and no retained retry/replay population for the full intent-retention and maximum redelivery window; attach worker cohort/image and producer-gate observations | Remove only the run-level union branch and candidate fixture; the candidate fixture must then produce the documented unsupported-shape failure while the predecessor fixture and node-context integration paths remain green | First set `FAILURE_NOTIFICATION_RUN_TIMEOUT_CONTEXT_ENABLED=false` on every R2 coordinator, then roll back only to the recorded R1 dual-reader artifact. Do not roll back to R0 or rewrite stored context while any candidate-shape intent, outbox row, delivery, retry, or replay can remain | Failure-notification owner / 2026-11-30 |
...
| Database rolling head | Published migrations through `EXPECTED_MIGRATION_HEAD` in [database readiness](../../packages/database/src/platform/readiness.ts), prior-head upgrade behavior, and role/function signatures | Migration 0000 / open-ended; current head and execution modes follow [the migration execution plan](../../packages/database/migrations/migration-execution-plan.json) | Migration, API, worker, maintenance, operator, dispatcher, and lifecycle roles | Migration-head unit tests; prior-head and compatibility-rollout integration suites; readiness signature checks | Successful mixed-version rollout and rollback rehearsal from the immediately prior production head, with backup/PITR evidence | Apply every published migration to a disposable prior-head database; old/new readiness probes and compatibility cohorts must match their declared support | Restore from the rehearsed backup or redeploy the compatible predecessor artifact; published migration files are never edited or removed | Database migration owner / each migration change |

## Release-cohort coverage

The [node-catalog registry](../../packages/node-catalog/src/registry.ts) owns
the authoritative `PLATFORM_RELEASE_COHORTS` list. Each cohort
