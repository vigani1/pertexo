...
| Scheduled workflows | Define a Schedule node in a draft and preview its next run times; after publication, inspect trigger health, the next run times and the retained run history, and enable/disable it. Workers handle timezone-aware due starts and trigger reconciliation. | [schedules](../apps/api/src/schedules/), [worker triggers](../apps/worker/src/triggers/) |
