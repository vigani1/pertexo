...
The current-status document now delegates mutable checkpoint decisions to this
tracker and explicitly labels older completion/verification statements as
historical evidence. Live migration-head and release-epoch guidance links to
the readiness contract, migration execution plan and node-catalog registry,
instead of duplicating values that drift on each release. The dependency
runbook references the executable `--audit-level` policy and distinguishes it
from the image scanner's separate threshold.

The documentation gate retains its Markdown-link and audit-tree checks and
adds focused operational-reference checks. Controlled negatives reject a stale
readiness link, copied migration head, copied release maximum and copied
dependency severity. Historical scalar evidence remains accepted. Primary
verification: all 12 documentation tests, repository link/tree validation,
scoped ESLint and formatting pass. This gate checks these known drift patterns,
not the factual completeness of every operational statement; the remaining
coverage qualifications and plan-to-evidence reconciliation stay open.

### IWA-14 — HTTP telemetry privacy

HTTP and Undici instrumentation now strips query, fragment and URL userinfo
before recording request attributes. Error events retain only bounded
categorical error codes/names; raw messages, stacks and status text are not
exported. This covers normal requests, connection refusal, malformed URLs and
injected transport errors containing multiple userinfo separators and query
values with spaces. Paths, methods, safe authority, response status and error
classification remain observable.
