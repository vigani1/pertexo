...
const dialogViewportVariants = cva(
  'fixed inset-0 z-50 flex overflow-y-auto overscroll-contain',
  {
    variants: {
      placement: {
        // Top-anchored: a message appearing inside grows the dialog
        // downward instead of moving its title.
        center: 'items-start justify-center px-4 pt-[max(1rem,14svh)] pb-4',
...
const dialogContentVariants = cva(
  'lens relative w-full overflow-y-auto shadow-2xl outline-none transition-[transform,opacity] motion-reduce:transition-none',
  {
    variants: {
      placement: {
        center:
          'max-h-[calc(86svh-1rem)] max-w-lg rounded-xl p-6 ease-unspool data-[ending-style]:scale-95 data-[ending-style]:opacity-0 data-[starting-style]:scale-95 data-[starting-style]:opacity-0',
