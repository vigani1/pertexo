...
export type NodeTestInputSource =
  | Readonly<{ kind: 'manual'; value: unknown }>
  | Readonly<{ kind: 'prior_preview'; previewRunId: string }>;
    ...
    body: nodeTestRequestSchema.parse({
      ...
      input:
        input.source.kind === 'manual'
          ? { kind: 'manual', value: input.source.value }
          : { kind: 'prior_preview', previewRunId: input.source.previewRunId },
