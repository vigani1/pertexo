...
| Date and time  | `components/ui/date-time-field.tsx`, `components/ui/calendar.tsx`         | `DateTimeField`: a day from a `Calendar` lens (days before today disabled; In 1 hour, Tomorrow 09:00, In 1 week), a typed 24-hour time and Clear, never the browser's native picker. Its value is local `datetime-local`-shaped text, so run and replay deadlines validate as before |
