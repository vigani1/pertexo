...
beforeAll(async () => {
  ...
  await copyFile(
    path.join(MIGRATIONS_DIRECTORY, '0065_operator_run_replay.sql'),
    path.join(priorDirectory, '0065_operator_run_replay.sql'),
  );
  await copyFile(
    path.join(MIGRATIONS_DIRECTORY, '0066_operator_maintenance_rerun.sql'),
    path.join(priorDirectory, '0066_operator_maintenance_rerun.sql'),
  );
  await copyFile(
    path.join(
      MIGRATIONS_DIRECTORY,
  ...
  await copyFile(
    path.join(MIGRATIONS_DIRECTORY, '0070_preview_execution_deadline.sql'),
    path.join(priorDirectory, '0070_preview_execution_deadline.sql'),
  );
  await copyFile(
  ...
  await copyFile(
    path.join(MIGRATIONS_DIRECTORY, '0073_transient_data_retention.sql'),
    path.join(priorDirectory, '0073_transient_data_retention.sql'),
  );
  await copyFile(
    path.join(MIGRATIONS_DIRECTORY, '0074_retention_schedule_state_rls.sql'),
    path.join(priorDirectory, '0074_retention_schedule_state_rls.sql'),
  );
  await expect(
    migrateDatabase(migrationConfig, priorDirectory),
  ).resolves.toEqual([
