...
const allowedReferencedSeries = [
  ...
  'pertexo_worker_process_start_time_seconds',
...
const requiredEmitterMetrics = [
  ...
  'pertexo.worker.process.start_time',
] as const;
...
async function readOperationsAssets() {
  const [
    alerts,
    dashboardText,
    collector,
  ...
  return {
    alerts,
    collector,
    dashboard: JSON.parse(dashboardText) as Dashboard,
    dashboardText,
    datasource,
    emitters,
    prometheus,
...
const operationsAssets = readOperationsAssets();
...
describe('operations observability assets', () => {
  ...
  it('links every textually discovered alert to its runbook section', async () => {
  ...
  it('keeps the textually referenced metric inventory explicit', async () => {
    const { alerts, dashboardText } = await operationsAssets;
    ...
    expect([...referencedSeries].sort()).toEqual(
      [...allowedReferencedSeries].sort(),
    );
  });

  it('finds every required metric name in its owning emitter source', async () => {
    const { emitters } = await operationsAssets;

    for (const emittedMetric of requiredEmitterMetrics) {
  ...
  it('keeps collector, Prometheus, and Grafana provisioning linked', async () => {
    const { collector, datasource, emitters, prometheus, provisioning } =
      await operationsAssets;
    ...
    expect(collector).not.toContain('key: service.instance.id');
    expect(emitters).toContain("'service.instance.id'");
