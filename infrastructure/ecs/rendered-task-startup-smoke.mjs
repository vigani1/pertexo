...
async function main() {
  ...
  const databaseHost =
    process.env.IWA02_DATABASE_HOST ?? 'host.docker.internal';
  ...
  const runId = smokeRunIdentifier();
  const containerOwner = createContainerOwner((args) => docker(args));
  const renderRootOwner = await acquireOwnedResource({
    acquireDirectory: () =>
      mkdtemp(resolve(tmpdir(), 'pertexo-iwa02-rendered-')),
    removeDirectory: (directory) =>
      rm(directory, { recursive: true, force: true }),
    acquire: async (owner) => owner.directory,
  });
  const temporaryRoot = renderRootOwner.value;
  ...
  try {
    const manifest = JSON.parse(
      await readFile(
        resolve(root, 'infrastructure/ecs/workloads.json'),
        'utf8',
      ),
    );
    ...
    await startWorker(
      ...
      containerOwner,
      runId,
      manifest.workloads.worker.healthCheck,
    );
  ...
  async function render(directory) {
    await runSmokeCommand(process.execPath, [renderer, directory], {
      cwd: root,
      env: renderEnvironment,
      label: 'task-definition renderer',
      timeoutMs: 30_000,
    });
  }

  async function resolveImage(localImage) {
    const inspection = JSON.parse(
      (
        await runSmokeCommand('docker', ['image', 'inspect', localImage], {
          cwd: root,
    ...
    const resolved = JSON.parse(
      (
        await runSmokeCommand('docker', ['image', 'inspect', reference], {
          cwd: root,
          label: 'digest-qualified image inspection',
          timeoutMs: 30_000,
        })
      ).stdout,
    )[0];
    ...
    return Object.freeze({
      ...
      source:
        repositoryDigest === undefined
          ? 'local-image-id-not-registry-attested'
  ...
  function imageRepository(localImage) {
    ...
    const withoutDigest =
      digestSeparator === -1
        ? localImage
        : localImage.slice(0, digestSeparator);
  ...
  function localFixtureValues(telemetryPort, objectStorePort) {
    ...
    return {
      ...
      DATABASE_WORKER_URL: postgresUrl(
        'pertexo_worker',
  ...
  function resolveTaskEnvironment(
    ...
    const injectedNames = (container.secrets ?? []).map(
      ({ name, valueFrom }) => {
        ...
        if (
          ...
          throw new Error(
            `${workloadName} renders undeclared injection ${name}`,
          );
  ...
  async function assertRoleConfigurationNegative(
    environment,
    caPath,
    runId,
    containerOwner,
    ...
    const containerId = await containerOwner.createAndStart(args);
    const waited = await docker(['wait', containerId]);
    const logs = await docker(['logs', containerId]);
    if (
      waited.stdout.trim() !== '1' ||
      !`${logs.stdout}${logs.stderr}`.includes(
        'Connection KMS configuration is incomplete',
      )
    )
      throw new Error(
  ...
  async function startApi(
    ...
    const name = `iwa02-rendered-api-${cohort}-${runId}`;
    ...
    const containerId = await containerOwner.createAndStart(args);
    const portOutput = await docker(['port', containerId, '3000/tcp']);
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          ...
          process.stdout.write(
            `IWA02 API ready: cohort=${cohort} container=${containerId.slice(0, 12)} port=${port}.\n`,
          );
          return;
        }
      } catch {
  ...
  async function startWorker(
    ...
    const name = `iwa02-rendered-worker-${cohort}-${runId}`;
    const args = dockerTaskArguments(name, environment, caPath, []);
    args.push(resolvedImage.reference, ...task.containerDefinitions[0].command);
    const containerId = await containerOwner.createAndStart(args);
    ...
    while (Date.now() < deadline) {
      ...
      const healthy = docker(
        renderedHealthCommand(
          task.containerDefinitions[0].healthCheck?.command,
          expectedHealthCheck,
          containerId,
        ),
      ...
      if (await healthy) {
        process.stdout.write(
          `IWA02 worker ready: cohort=${cohort} container=${containerId.slice(0, 12)}.\n`,
        );
        return;
      }
      await delay(250);
  ...
  function dockerTaskArguments(name, environment, caPath, extra) {
    const args = [
      '--name',
      name,
      '--init',
  ...
  async function docker(args, { timeoutMs = 30_000 } = {}) {
    return runSmokeCommand('docker', args, {
      cwd: root,
      label: `docker ${args[0] ?? 'command'}`,
      timeoutMs,
    });
  }
  ...
  async function createTelemetryServer() {
    ...
    return {
      ...
      close: () => closeServerResources(server, sockets, []),
    };
  }

  async function createObjectStoreBridge() {
    const owned = await acquireOwnedResource({
      acquireDirectory: () => mkdtemp(resolve(tmpdir(), 'pertexo-iwa02-tls-')),
      removeDirectory: (directory) =>
        rm(directory, { recursive: true, force: true }),
      acquire: async (owner) => {
        const keyPath = resolve(owner.directory, 'local.key');
        const caPath = resolve(owner.directory, 'local-ca.pem');
        await runSmokeCommand(
          ...
          {
            cwd: root,
            label: 'local TLS certificate generation',
            timeoutMs: 30_000,
          },
        );
        const sockets = new Set();
        ...
        const server = createHttpsServer(
          ...
          (request, response) =>
            bridgeObjectStoreRequest(request, response, upstreamRequests),
        );
        trackServerSockets(server, sockets);
        owner.registerCleanup(() =>
          closeServerResources(server, sockets, upstreamRequests),
        );
        ...
        await listen(server);
        return { caPath, port: server.address().port };
      },
    });
    return { ...owned.value, close: owned.close };
  }

  function bridgeObjectStoreRequest(request, response, upstreamRequests) {
    const upstream = httpRequest(
      ...
      (upstreamResponse) => {
        const failResponse = (error) => {
          process.stderr.write(
            `IWA02 local object-store bridge response failed: ${error instanceof Error ? error.message : String(error)}\n`,
          );
